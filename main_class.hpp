...
// do instance rendering for sprites, that are the same
// do instance rendering for sprites, that are the same
// do instance rendering for sprites, that are the same
// do instance rendering for sprites, that are the same
// do instance rendering for sprites, that are the same
...
private:
    ...
    GameModelCreationInfo globalModelCreationInfo = {
         &device,
         &physicalDevice,
         &surface,
         &graphicsQueue,
    ...
    // Careful about alignment requirements
    ...
    void initVulkan() {
        ...
        auto elapsedSeconds = std::chrono::duration_cast<std::chrono::milliseconds>(now - then).count();
    ...
    void cleanup() {
        endSingleTimeCommands(setupCommandBuffer, device, CommandPool, graphicsQueue);
        ...
        // replace with function to destroy all models in a scene (when I implement a scene)
        model->destroy();
    ...
    void loadModels() {
        model = GameModel::createModelFromFile(globalModelCreationInfo, MODEL_PATH, TEXTURE_PATH);
    }
    ...
    void createDepthResources() {
        ...
        transitionImageLayout(depthImage, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, 1, setupCommandBuffer);
    ...
    void createTextureSampler() {
        ...
        samplerInfo.compareEnable = VK_FALSE; // this is relevant for shadow mapping
    ...
    void createTextureImage() {
        ...
        createBuffer(device, physicalDevice, surface, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        ...
        transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels, setupCommandBuffer);
        ...
        generateMipmaps(textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, mipLevels, setupCommandBuffer);
    ...
    void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels, VkCommandBuffer commandBuffer) {
    ...
    void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels, VkCommandBuffer commandBuffer) {
        VkImageMemoryBarrier barrier{};
        barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    ...
    void createUniformBuffers() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            createBuffer(device, physicalDevice, surface, bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        //for (int i = 0; i < 4; i++) {
            ...
            pushData.offset = { 0.0f, 0.0f, 0.0f * 1.25f };
            pushData.color = { 0.0f, 0.0f, 0.0f };

            vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT |
                VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(SimplePushConstantData), &pushData);
    ...
    void createGraphicsPipeline() {
        auto fragShaderCode = readFile("CompiledShaders/fragShader.spv");
        auto vertShaderCode = readFile("CompiledShaders/vertShader.spv");
        ...
        vertShaderStageInfo.pSpecializationInfo = nullptr; // Used to specify constants
        ...
        auto bindingDescription = GameModel::Vertex::getBindingDescription();
        auto attributeDescriptions = GameModel::Vertex::getAttributeDescriptions();
    ...
    void createLogicalDevice() {
        ...
        // Compatibility only, not necessary in newer Vulkan versions
        if (enableValidationLayers) {
    ...
    int rateDeviceSuitability(VkPhysicalDevice device) {
        ...
        score += deviceProperties.limits.maxImageDimension2D / 8;

        // Prefer device with both drawing and presentation capabilities
    ...
    bool checkDeviceFeaturesSupport(VkPhysicalDevice device) {
        ...
        return deviceFeatures.geometryShader && deviceFeatures.depthClamp && deviceFeatures.logicOp && deviceFeatures.samplerAnisotropy;
    }
