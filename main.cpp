...
private:
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        float zoomFactor = pow(1.1f, keys.scrollOffset);

        if (glfwGetKey(window, keys.lookRight) == GLFW_PRESS) offset.x += 0.1f / zoomFactor;
        if (glfwGetKey(window, keys.lookLeft) == GLFW_PRESS) offset.x -= 0.1f / zoomFactor;
        if (glfwGetKey(window, keys.lookUp) == GLFW_PRESS) offset.y -= 0.1f / zoomFactor;
        if (glfwGetKey(window, keys.lookDown) == GLFW_PRESS) offset.y += 0.1f / zoomFactor;
